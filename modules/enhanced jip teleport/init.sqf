...
if (!isDedicated) then {
	...
	if ((_targetUnit distance player) >  LEADERDISTANCE) then {
		_teleportAction = player addAction ["Teleport to Squad Leader", "modules\enhanced jip teleport\teleportAction.sqf", [_targetUnit]];
